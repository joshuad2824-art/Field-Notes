...
- **The band at the bottom of the phone: five goes, and the fifth is `body`.** Worth keeping the whole sequence, because the lesson is entirely about method. (1) `<meta name="theme-color">` — set perfectly, changed nothing. (2) The app was measuring itself short; `visualViewport.height`, then `window.innerHeight`. Both real bugs, both fixed, band survived. (3) The manifest's `theme_color`, on the argument that the strip is outside the web view where no document can reach — which would have meant one colour for the whole app forever. (4) The *canvas*: a background on `html`, moved by `setEdgeColor` to whatever is at the bottom of the screen. Genuinely right, and it changed nothing either. (5) **`body`.** WebKit paints that strip from `body`, not from `html`, and `body` had been sitting on the frame's `#142a2b` the whole time — which is exactly the colour of the band in every screenshot of it. `setEdgeColor` now sets both.
