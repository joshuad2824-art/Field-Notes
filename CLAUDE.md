...
- **A press inside writing that is already selected starts a drag, not a selection — and that reads as the mouse refusing to work.** Every browser holds the selection still until an HTML5 drag actually begins, and a drag that ends back inside itself changes nothing. So the second attempt to pick a phrase — which is what anyone does when the first pick was a word too long — quietly did nothing at all. Reported from a Windows machine as "sometimes the mouse just completely refused to highlight text", and it is: the mouse was doing something else. A `mousedown` handler now lets go of the selection first, so a press always starts a new one. What it costs is dragging text about with the mouse, which nothing in this app has ever advertised and which, on a page of writing, is mostly a way to move a paragraph somewhere by accident. Shift-click, double-click-drag and the picture plate's own drag are all untouched — the handler stands aside for a modifier, for a repeat click, and for anything CodeMirror never sees.
...
rotted: **503 assertions in six files**, all green. `tests/editor.mjs` has 275 covering the editor,
...
`tests/spreadsheet.mjs` has 55, and it asks one question over and over: **does the
