...
- **On a phone in portrait the strip is the manifest's, and that is the third mechanism this band has been blamed on.** Worth writing the sequence down, because the lesson is about the method rather than the pixel. It was `<meta name="theme-color">`, which turned out to be set perfectly and to change nothing. Then it was the canvas, which was genuinely wrong as well and is now right — a background on `html` is painted across the whole window and ours was propagating up from `body`, the frame. The band survived both. What settles it is that the band is there in portrait and **gone in landscape**, where the page foot sits flush on the bottom edge and `env(safe-area-inset-bottom)` is zero: in portrait the leaf is already painting every pixel it is allowed to and the band is *outside the web view*, where no document can reach. The only colour that region can be told is `theme_color` in the manifest, read once at launch — so that is now `#051B1C`, the night leaf, which is also what the list's foot lands at and what the splash already was. index.html opens on the same value and `npm run check` holds the two together, because whichever one the system reads it should get the same answer. **The cost is honest and worth stating: it is one colour for the whole app, so a *paper* page in portrait will show a dark strip under a cream leaf.** There is no second colour to give it. **Half of it is now measured rather than argued.** Settings' screen diagnostics, read off the phone in portrait: `innerHeight`, `visualViewport.height` and `clientHeight` all agree at **894**, `#root` and `body` both reach 894, `--app-height` is unset and `--browser-bottom` is 0 — and `screen.height` is **956**. The app fills the view exactly and the view is 62pt short of the screen, which is the iPad's 712-of-744 again and larger. So the strip is provably not ours to paint, and the manifest is provably the only thing left that can. What is *not* yet proven is that iOS repaints it when the manifest changes — that needs a relaunch and probably a re-add of the Home Screen icon before it can be believed either way. The panel now prints `outside the view` as its own line so the next person subtracts nothing.
- **A change to anything in the shell needs `VERSION` in `public/sw.js` bumped.** The manifest is cached cache-first and only re-fetched when the worker reinstalls, so a manifest edit on an unchanged version never reaches a phone at all. It cost the first attempt at the line above.
- **The strip is coloured from what is actually at the bottom, and the *canvas* is what colours it everywhere the manifest doesn't — not `<meta name="theme-color">`.** That was wrong for a release. The meta was being set perfectly, on every screen, and changing nothing: a background on `html` is painted across the whole window including the strip outside the layout viewport, and left unset it propagates up from `body` — which is the frame. So the frame's `#142a2b` was painted under every screen whatever the meta said. It went unnoticed on the iPad because the frame's teal is what the strip was meant to be there; it shows on a phone the moment a page's own stock is anything else, as a cream leaf on a teal band. `html` now has its own background and `setEdgeColor` moves it. The meta is still set, because other browsers do use it. The rule was always "whatever is at the bottom of the screen" — but only the page screen ever did the work. Everywhere else it was hardcoded to the frame's teal, and on a phone the bottom of the screen is the list's foot, which is `rgba(5, 27, 28, .92)` and lands at about `#061c1d`. Twenty points darker in every channel, which is a plainly visible band. `surfaceColor` composites an element's real background — `getComputedStyle` reports the colour that was asked for, not the one that landed — and the list sets the strip from its own foot whenever it is the whole window. Docked, it doesn't: the foot is then one column of three and has no claim on the width of the screen.
...
- **The iOS keyboard.** Two rounds on a real phone found two separate moves: iOS
  ...
  **canvas** — `html`'s background, or `body`'s when `html` hasn't got one —
  ...
  That was `<meta name="theme-color">` until August 2026 and the meta never
  had anything to do with it. See the open item above; it is the fourth go at
  this band and the first one that moves the pixel that was actually wrong.
