...
- Real editor core. The spike re-renders the whole document on every keystroke and hides markers with `font-size:0`; that's a prototype technique. Phase 1 needs markers treated as atomic units so backspacing past a hidden `**` doesn't feel broken. Evaluate CodeMirror 6 before writing one.
