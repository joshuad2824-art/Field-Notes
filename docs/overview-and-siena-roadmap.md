...
From Siena is a durable inbox. The newest note appears in full on Overview. Active reminders due today or earlier appear there until completed, as do recent unseen task updates. Each notebook can have one pinned Reminders page, created with its first reminder. That page and Overview read the same reminder rows. Checking off a reminder on either screen removes it from the active list and keeps it in the completed history in `/from-siena`. **Mark seen** only acknowledges reading. Merely opening either screen does not acknowledge anything. There are no device notifications.
...
The MCP function adds `list_siena_items` and `create_siena_item`. Publishing requires the existing linked account. For reminders, `create_siena_item` takes a due timestamp and the chosen notebook ID, creates or reuses the pinned Reminders page, and publishes one reminder row. A `source_key` makes retries idempotent. Its tool description asks Siena to use this path for reminders rather than creating an ordinary checklist page. No automation has been created to publish items yet.
